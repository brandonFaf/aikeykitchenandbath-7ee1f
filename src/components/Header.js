                    ...
                    <div className="site-nav-wrap">
                      <div className="site-nav-inside">
                          ...
                          {_.map(_.get(this.props, 'data.config.header.nav_links', null), (action, action_idx) => {
                              ...
                              return (
                                <li key={action_idx} className={classNames('menu-item', {'current-menu-item': pageUrl === actionUrl, 'menu-button': _.get(action, 'style', null) === 'button'})}>
                                  <Link className={classNames({'button': _.get(action, 'style', null) === 'button'})} href={withPrefix(_.get(action, 'url', null))}{...(_.get(action, 'new_window', null) ? ({target: '_blank', rel: 'noopener'}) : null)}>{_.get(action, 'label', null)}</Link>
                                </li>
                              )
                          })}
                        </ul>
                        ...
                        <div className="social-links">
                          ...
                          action && (
                          <Link key={action_idx} className={classNames({'button button-icon': _.get(action, 'style', null) === 'icon'})} href={withPrefix(_.get(action, 'url', null))}{...(_.get(action, 'new_window', null) ? ({target: '_blank', rel: 'noopener'}) : null)}>
